...
class SingleShoe extends Component {
  ...
  fetchShoeById = () => {
    axios
      .get(`https://be-scheuster.herokuapp.com/api/shoes/${this.props.shoe_id}`)
      .then(({ data }) => {
        this.setState({
          shoe_info: data.shoes[0],
          isLoading: false,
          error: null,
        });
      })
  ...
  render() {
    ...
    return (
      <main className="single_page_shoe">
        ...
        <br />
        <body>
        <h4><strong>Description:</strong> {shoe_info.description}</h4>
        <h4><strong>Sizing Info:</strong> {shoe_info.sizing_info}</h4>
        <h4><strong>Number in stock:</strong> {shoe_info.stock_number}</h4>
        <br />
        </body>
      </main>
    );
